<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:81:"F:\Work space\test\public_html/../application/admin\view\goods\wdl_cate_edit.html";i:1618983318;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<form class="layui-form" id="form" method="post">
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-block"><span class="require-field">*</span>
      <input type="text" name="name" placeholder="请输入文章分类名称" value="<?php echo $row['name']; ?>" class="layui-input must">
    </div>
  </div>
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-block"><span class="require-field">*</span>
      <select name="pid">
        <option value="0" <?php if($row['pid'] == 0): ?>selected<?php endif; ?>>作为顶级分类</option>
      ...
      <option value="<?php echo $vo['id']; ?>" <?php if($vo['id'] == $row['id']): ?>disabled<?php endif; if($row['pid'] == $vo['id']): ?>selected<?php endif; ?>><?php echo $vo['name']; ?></option>
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-block">
      <input type="text" name="icon" placeholder="请输入分类图标" value="<?php echo $row['icon']; ?>" class="layui-input layui-hide">
      <img <?php if(!(empty($row['icon']) || (($row['icon'] instanceof \think\Collection || $row['icon'] instanceof \think\Paginator ) && $row['icon']->isEmpty()))): ?>src="<?php echo $row['icon']; ?>"<?php endif; ?> class="cate-image-preview"/>
    </div>
