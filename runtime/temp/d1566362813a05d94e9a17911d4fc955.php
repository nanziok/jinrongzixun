<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:79:"F:\Work space\test\public_html/../application/admin\view\guanggao\wdl_edit.html";i:1617415141;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<form class="layui-form layui-form-pane" action="<?php echo url('wdl_index_post'); ?>" id="form" >
    ...
    <div class="layui-input-block">
      <select name="type" >
        ...
        <option value="1" <?php if($data['type'] == 1): ?>selected<?php endif; ?>>菜单链接</option>
		    <option value="2" <?php if($data['type'] == 2): ?>selected<?php endif; ?>>案例</option>
		    <option value="3" <?php if($data['type'] == 3): ?>selected<?php endif; ?>>网点</option>
		    <option value="6" <?php if($data['type'] == 6): ?>selected<?php endif; ?>>外链</option>
      </select>
    </div>
  </div>
  <div class="layui-form-item">
  ...
  <p class="red" style="margin-bottom:15px;">温馨提示：<br>选择菜单链接，需要技术指导。选择案例，需要输入案例ID。<br>
  选择网点，需要输入网点ID。选择外联，需要输入带http的链接。</p>
  <div class="layui-form-item">
